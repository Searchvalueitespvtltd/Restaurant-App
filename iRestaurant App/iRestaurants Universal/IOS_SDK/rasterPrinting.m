...
- (IBAction)selectFont:(id)sender
{
    ActionStringDoneBlock done = ^(ActionSheetStringPicker *picker, NSInteger selectedIndex, id selectedValue)
    {
        ...
        [prefs setObject:selectedFont forKey:@"PrinterFont"];
    };
...
- (IBAction)selectFontStyle:(id)sender
{
    ActionStringDoneBlock done = ^(ActionSheetStringPicker *picker, NSInteger selectedIndex, id selectedValue)
    {
        ...
        uitextview_texttoprint.font = [self getSelectedFont:1];
        NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
        [prefs setObject:selectedFontStyle forKey:@"PrinterFontStyle"];
    };
...
- (IBAction)selectPrinterSize:(id)sender
{
    ActionStringDoneBlock done = ^(ActionSheetStringPicker *picker, NSInteger selectedIndex, id selectedValue)
    {
        ...
        [buttonPrinterSize setTitle:selectedValue forState:UIControlStateNormal];
         NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
        [prefs setObject:selectedPrinterSize forKey:@"PrinterSize"];
    };
...
- (UIFont *)getSelectedFont:(int)multiple;
{
    int fontIndex = selectedFontStyle;
    if (fontIndex > array_fontStyle.count - 1) {
        fontIndex = array_fontStyle.count - 1;
    }
...
- (IBAction)printRasterText
{
    ...
    portName=[NSString stringWithFormat:@"%@",[prefs stringForKey:@"PrinterPortName"]];
    portSettings=[NSString stringWithFormat:@"%@",[prefs stringForKey:@"PortSettings"]];
    ...
    [prefs setObject:imageToPrint forKey:@"imageToPrint"];
    [prefs setBool:switchCompression.on forKey:@"switchCompression"];
    [prefs setBool:switchPageMode.on forKey:@"switchPageMode"];
